(()=>{
  ...
  const DETOUR_RADII_METERS=[450,900,1800,3500,6500,10000,16000];
  const DETOUR_BEARINGS=[0,45,90,135,180,225,270,315];

  function cameraKey(cam){return String(cam.node_id||cam.id||`${cam.lat},${cam.lng}`)}
  function exclusionMeters(cam){
    const text=`${cam.label||''} ${cam.type||''} ${cam.source||''}`.toLowerCase();
    if(/falcon\s*lr|long[- ]?range|\blr\b/.test(text))return 50;
    if(/falcon\s*sr|short[- ]?range|\bsr\b/.test(text))return 22;
    if(/flock|alpr|lpr|license plate|plate reader/.test(text))return 35;
    return 30;
  }
  ...
  function auditRoute(route){
    ...
    for(const pt of points){for(const cam of cameraLocations||[]){
      ...
      const limit=exclusionMeters(cam);
      if(d<=limit){const key=cameraKey(cam),prior=hitMap.get(key);if(!prior||d<prior.distance)hitMap.set(key,{camera:cam,distance:d,exclusion:limit})}
    }}
    ...
    for(let depth=0;depth<3 && requests<MAX_DETOUR_REQUESTS;depth++){
      ...
      for(const candidate of frontier){
        ...
        for(const blocker of blockers){
          ...
          for(const radius of DETOUR_RADII_METERS){for(const bearing of DETOUR_BEARINGS){
            if(requests>=MAX_DETOUR_REQUESTS)break;
            const wp=destinationPoint(lat,lng,bearing,radius),via=[...(candidate.via||[]),wp];
            try{const routes=await requestRoutes([start,...via,dest],false);requests++;for(const r of routes){const item={route:r,audit:auditRoute(r),via};tested.push(item);if(item.audit.clear)return {best:item,requests,tested};next.push(item)}}catch(e){requests++;console.debug('[GhostLane] detour candidate failed',e.message)}
          }if(requests>=MAX_DETOUR_REQUESTS)break}if(requests>=MAX_DETOUR_REQUESTS)break
        }if(requests>=MAX_DETOUR_REQUESTS)break
      }
      frontier=rankAudited(next).slice(0,4);if(!frontier.length)break;
    }
  ...
  async function strictCameraFreeSearch(){
    ...
    if(typeof showHudToast==='function')showHudToast(`Zero-camera search • using detection-range exclusion zones for ${cameraLocations.length} mapped nodes…`);
    if(typeof speakAlert==='function')speakAlert('Calculating strict zero camera route. Detection range zones active.');
    try{
      ...
      if(!result.best){
        clearExistingRoute();const least=rankAudited(result.tested)[0],count=least?.audit?.hits?.length||0;
        const msg=`GhostLane searched ${result.requests} route/detour candidates and still could not verify a route outside every mapped camera's estimated detection zone${count?`. The best remaining candidate still intersects ${count} mapped camera zone${count===1?'':'s'}`:''}. Navigation was not started.`;
        if(typeof showHudModal==='function')showHudModal('Zero-Camera Route Not Verified',msg,{tone:'danger'});if(typeof speakAlert==='function')speakAlert('Zero camera route could not be verified. Navigation blocked.');return;
      }
      ...
      if(typeof speakAlert==='function')speakAlert('Zero camera route verified against estimated camera detection ranges.');
      if(routeSteps[0]&&typeof speakAlert==='function')setTimeout(()=>speakAlert(instructionFor(routeSteps[0])),900);
      console.info('[GhostLane] range-aware zero-camera route accepted',{version:VERSION,requests:result.requests,via:best.via,audit:best.audit,distance:r.distance,duration:r.duration});
  ...
  console.info(`[GhostLane] range-aware zero-camera Shadow Route ${VERSION} active`);
})();